import type { CanonicalMuscle, ExerciseMuscleInfo } from '../types';
...
function normalizeEmphasis(value?: string): string | undefined {
  const normalized = normalizeText(value);
  if (!normalized) return undefined;
  return EMPHASIS_SYNONYMS[normalized] ?? normalized;
}

function inferResolution(rawMuscle: string, rawEmphasis?: string): CanonicalResolution {
  ...
  if (
    (source.includes('bicep') || source.includes('bícep') || source.includes('braquial') || source.includes('braquiorradial')) &&
    !source.includes('femoral')
  ) {
    return {
      muscle: 'Bíceps',
  ...
  if (source.includes('pectoral') || source.includes('pecho') || source.includes('chest')) {
    return {
      muscle: 'Pectorales',
      emphasis:
        emphasis === 'superior' || emphasis === 'medio' || emphasis === 'inferior'
          ? emphasis
          : undefined,
    };
  }
...
export function getCanonicalMuscleList(): CanonicalMuscle[] {
  return [...CANONICAL_MUSCLES];
}

export function normalizeCanonicalMuscle(rawMuscle: string, rawEmphasis?: string): CanonicalResolution {
  return inferResolution(rawMuscle, rawEmphasis);
}

export function normalizeInvolvedMuscles(
  involvedMuscles: ExerciseMuscleInfo['involvedMuscles'] | undefined
): ExerciseMuscleInfo['involvedMuscles'] {
...
export function getExercisePrimaryDisplayMuscles(
  exercise: Pick<ExerciseMuscleInfo, 'involvedMuscles'>
): string[] {
