...
const BatteryRing: React.FC<{ label: string; value: number; colorClass: string; icon: React.ReactNode }> = ({ label, value, colorClass, icon }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center p-3 bg-[#ECE6F0] border border-[#E6E0E9] rounded-2xl relative overflow-hidden group">
            ...
            <span className="text-[8px] text-[#49454F] font-black uppercase tracking-widest mt-1">{label}</span>
        </div>
    );
};
...
const AuditList: React.FC<{ logs: BatteryAuditLog[]; title: string; color: string }> = ({ logs, title, color }) => (
    <div className="mb-6">
        ...
        {logs.length === 0 ? (
            <div className="text-[10px] text-zinc-600 bg-black p-3 rounded-xl border border-[#E6E0E9] text-center">Batería sin desgaste reciente.</div>
        ) : (
            <div className="space-y-2">
                {logs.map((log, i) => (
                    <div key={i} className="flex justify-between items-center bg-[#FEF7FF] p-3 rounded-xl border border-[#E6E0E9]">
...
const SystemBatteryWidget: React.FC = () => {
    ...
    return (
        <>
            {/* WIDGET HOME */}
            <div onClick={() => setIsAuditModalOpen(true)} className="bg-[#FEF7FF] border border-[#222] rounded-[2rem] p-5 cursor-pointer hover:border-white/20 transition-all shadow-xl relative overflow-hidden group">
                ...
                <div className="flex justify-between items-center mb-5 relative z-10">
                    <div>
                        ...
                        <p className="text-[8px] text-[#49454F] font-bold uppercase tracking-widest mt-1">Sistemas Biológicos</p>
                    </div>
                    <button className="text-[8px] bg-[#ECE6F0] border border-[#333] text-[#49454F] px-3 py-1.5 rounded-full font-black uppercase tracking-widest hover:text-white transition-colors">Auditar</button>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-[#E6E0E9] relative z-10 flex gap-3 items-start">
                    <InfoIcon size={16} className="text-[#49454F] shrink-0 mt-0.5" />
                ...
                {adaptiveCache && adaptiveCache.banister && (
                    <div className="mt-3 pt-3 border-t border-[#E6E0E9] relative z-10 flex items-start gap-2">
            ...
            <TacticalModal isOpen={isAuditModalOpen} onClose={() => setIsAuditModalOpen(false)} title="Auditoría de Sistemas" useCustomContent={true}>
                <div className="bg-[#050505] w-full max-w-md mx-auto h-[85vh] sm:h-[700px] flex flex-col text-white">
                    <div className="flex justify-between items-center p-5 border-b border-[#222] shrink-0 bg-black relative z-20">
                        ...
                        <button onClick={() => setIsAuditModalOpen(false)} className="text-[#49454F] hover:text-white transition-colors p-1 bg-zinc-900 rounded-full"><XIcon size={16}/></button>
                    </div>

                    {!isCalibrating && (
                        <div className="flex border-b border-[#222] shrink-0">
                            <button onClick={() => setAuditTab('logs')} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest transition-colors ${auditTab === 'logs' ? 'text-white border-b-2 border-white' : 'text-[#49454F] hover:text-zinc-300'}`}>Diagnóstico</button>
                            <button onClick={() => setAuditTab('precision')} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest transition-colors ${auditTab === 'precision' ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-[#49454F] hover:text-zinc-300'}`}>Precisión AUGE</button>
                        </div>
                    )}
                        ...
                        {isCalibrating ? (
                            <div className="bg-[#ECE6F0] border border-blue-500/30 p-5 rounded-2xl space-y-6 animate-fade-in shadow-[0_0_30px_rgba(59,130,246,0.1)]">
                                ...
                                <div className="flex gap-2 pt-2">
                                    <button onClick={() => setIsCalibrating(false)} className="flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest bg-zinc-900 text-[#49454F] hover:text-white">Cancelar</button>
                            ...
                            <>
                                ...
                                <div className="pt-4 border-t border-[#E6E0E9] flex justify-center">
                                    <button onClick={openCalibration} className="w-full bg-[#ECE6F0] border border-[#E6E0E9] text-white px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white hover:text-black transition-all flex items-center justify-center gap-2">
                            ...
                            <div className="space-y-6 animate-fade-in">
                                {adaptiveCache?.selfImprovement ? (
                                    <>
                                        <div className="bg-[#ECE6F0] border border-yellow-500/20 p-5 rounded-2xl">
                                        ...
                                        {adaptiveCache.selfImprovement.accuracy_by_system.length > 0 && (
                                            <div className="bg-[#ECE6F0] border border-[#E6E0E9] p-5 rounded-2xl">
                                                <h4 className="text-[10px] font-black uppercase tracking-widest text-[#49454F] mb-4">Precisión por Sistema</h4>
                                                <div className="space-y-3">
                                                    {adaptiveCache.selfImprovement.accuracy_by_system.map(sys => (
                                                        <div key={sys.system} className="flex justify-between items-center">
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                <span className="text-[9px] text-[#49454F] font-mono">R²={sys.r_squared.toFixed(2)}</span>
                                                                <span className="text-[9px] text-[#49454F] font-mono">MAE={sys.mae.toFixed(1)}</span>
                                    ...
                                    <div className="text-center py-12">
                                        ...
                                        <p className="text-[10px] text-[#49454F] font-bold uppercase tracking-widest">AUGE aún no tiene datos de auto-mejora</p>
