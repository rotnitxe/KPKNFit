    ...
    return (
        <div className="max-w-lg mx-auto py-6 px-4 space-y-6">
            <div className="text-center">
                ...
                <p className="text-[10px] text-[#49454F] mt-1">Configura hitos y duración de bloques</p>
            </div>
                        ...
                        <div key={i} className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="w-6 h-6 rounded bg-zinc-800 border border-[#E6E0E9] text-[#49454F] hover:text-white flex items-center justify-center text-xs font-bold"
                                >
                                    -
                                </button>
                                ...
                                <span className="text-[9px] text-[#49454F]">sem</span>
                            </div>
                        </div>
                    ))}
                    <div className="text-right">
                        <span className="text-[9px] text-[#49454F] font-bold">Total: {totalWeeks} semanas</span>
                    </div>
                </div>
            )}
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    <h4 className="text-[9px] font-black text-[#49454F] uppercase tracking-widest">Eventos Programados</h4>
                    <button
                        onClick={() => setShowForm(!showForm)}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-zinc-900 border border-[#E6E0E9] text-[9px] font-black text-[#49454F] uppercase tracking-widest hover:text-white transition-colors"
                    >
                ...
                {events.length > 0 ? (
                    events.map((ev, idx) => (
                        <div key={idx} className="flex items-center gap-3 bg-[#FEF7FF] border border-[#E6E0E9] rounded-xl p-3 group">
                    ...
                    <div className="text-center py-6 border border-dashed border-[#E6E0E9] rounded-xl bg-black/20">
                        <CalendarIcon size={20} className="text-zinc-700 mx-auto mb-1" />
                        <p className="text-[9px] text-[#49454F] font-bold">Sin eventos</p>
                    </div>
                )}
            </div>
            ...
            {/* Add event form */}
            {showForm && (
                <div className="bg-[#FEF7FF] border border-[#E6E0E9] rounded-2xl p-4 space-y-3 animate-fade-in">
                    <div className="flex items-center justify-between">
                        <h4 className="text-[10px] font-black text-white uppercase tracking-widest">Nuevo Evento</h4>
                        <button onClick={() => setShowForm(false)} className="p-1 text-[#49454F] hover:text-white">
                            <XIcon size={14} />
                        </button>
                    </div>
                    <div>
                        <label className="text-[8px] font-black text-[#49454F] uppercase tracking-widest block mb-1">Nombre</label>
                        <input
                            type="text"
                    ...
                    {isSimple ? (
                        <div>
                            ...
                            <input
                                ...
                                className="w-24 bg-black border border-[#E6E0E9] rounded-lg px-3 py-2 text-xs font-bold text-white text-center focus:ring-1 focus:ring-white/30"
                            />
                        </div>
                    ) : (
                        <div>
                            <label className="text-[8px] font-black text-[#49454F] uppercase tracking-widest block mb-1">Semana</label>
                            <input
                                type="number"
                                min={1}
