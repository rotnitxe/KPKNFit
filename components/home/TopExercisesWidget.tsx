    ...
    if (topExercises.length === 0) {
        return (
            <div className="w-full bg-[#FEF7FF] border border-[#E6E0E9] rounded-2xl p-4">
                ...
                <p className="text-[10px] text-[#49454F] font-mono">Sin datos en últimos 30 días</p>
            </div>
        );
    }
            ...
            <div className="space-y-2">
                {topExercises.map((ex, i) => (
                    <button
                        ...
                        className="w-full flex items-center justify-between py-2 px-3 rounded-lg bg-black/30 border border-[#E6E0E9] hover:border-white/15 transition-colors text-left group"
                    >
                        <div className="flex items-center gap-2 min-w-0">
                            <span className="text-[9px] font-mono text-[#49454F] w-4">{i + 1}</span>
                            <span className="text-[10px] font-bold text-white truncate">{ex.name}</span>
                        </div>
                        <div className="flex items-center gap-3 shrink-0">
                            <span className="text-[9px] font-mono text-[#49454F]">{ex.count} ses.</span>
                            {ex.maxWeight > 0 && (
