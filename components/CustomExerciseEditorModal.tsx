...
const CustomExerciseEditorModal: React.FC<CustomExerciseEditorModalProps> = ({ isOpen, onClose, onSave, existingExercise, preFilledName }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} useCustomContent={true}>
        <div className="flex flex-col bg-[#0a0a0a] text-white h-[90vh] sm:h-[650px] w-full max-w-lg mx-auto overflow-hidden">
            
            {/* Header Fijo */}
            <div className="flex justify-between items-center p-5 border-b border-[#222] bg-[#0a0a0a] shrink-0">
                <h2 className="text-lg font-black uppercase tracking-tight">{existingExercise ? "Editar Ejercicio" : "Nuevo Ejercicio"}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1 md:col-span-2">
                        ...
                        <input 
                            ...
                            className="w-full bg-[#111] border border-[#222] rounded-xl p-3 text-sm font-bold text-white focus:border-white outline-none transition-colors placeholder:text-zinc-700"
                        />
                    </div>
                    <div className="space-y-1">
                        ...
                        <input 
                            ...
                            placeholder="Ej. OHP"
                            className="w-full bg-[#111] border border-[#222] rounded-xl p-3 text-sm font-bold text-white focus:border-white outline-none transition-colors placeholder:text-zinc-700 text-center"
                        />
                    </div>
                </div>

                {/* Clasificación en Grid 2x2 compacta */}
                <div className="bg-[#111] border border-[#222] rounded-xl p-4 grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Patrón</label>
                        <select value={exercise.force} onChange={e => handleForceChange(e.target.value)} className="w-full text-xs font-bold bg-[#0a0a0a] border border-[#222] rounded-lg p-2 text-white outline-none focus:border-zinc-500">
                    ...
                    <div className="space-y-1">
                        ...
                        <select value={exercise.equipment} onChange={e => handleChange('equipment', e.target.value as any)} className="w-full text-xs font-bold bg-[#0a0a0a] border border-[#222] rounded-lg p-2 text-white outline-none focus:border-zinc-500">
                    ...
                    <div className="space-y-1">
                        ...
                        <select value={exercise.type} onChange={e => handleChange('type', e.target.value as any)} className="w-full text-xs font-bold bg-[#0a0a0a] border border-[#222] rounded-lg p-2 text-white outline-none focus:border-zinc-500">
                    ...
                    <div className="space-y-1">
                        ...
                        <select value={exercise.category} onChange={e => handleChange('category', e.target.value as any)} className="w-full text-xs font-bold bg-[#0a0a0a] border border-[#222] rounded-lg p-2 text-white outline-none focus:border-zinc-500">
                ...
                {/* Pregunta Clave AUGE */}
                <div className="bg-[#111] p-4 rounded-xl border border-[#222] flex justify-between items-center">
                    <div>
                ...
                {/* Perfil AUGE */}
                <div className="bg-[#111] p-4 rounded-xl border border-[#333] space-y-3">
                ...
                <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                        {exercise.involvedMuscles.map((inv, idx) => (
                            <div key={idx} className="flex items-center gap-2 bg-[#111] p-2 rounded-xl border border-[#222]">
                                ...
                                <select value={inv.role} onChange={e => handleInvolvedMuscleChange(idx, 'role', e.target.value)} className={`w-28 text-[9px] font-black uppercase bg-[#0a0a0a] border border-[#222] rounded p-1.5 outline-none ${inv.role === 'primary' ? 'text-white' : 'text-zinc-500'}`}>
                        ...
                        <button 
                            ...
                            className="w-full py-3 rounded-xl border border-dashed border-[#333] text-zinc-500 hover:text-white hover:border-zinc-500 bg-transparent transition-all text-xs font-bold flex items-center justify-center gap-2"
                        >
                ...
                {error && <p className="text-zinc-400 text-center text-xs font-bold bg-[#1a1a1a] border border-[#333] p-3 rounded-xl">{error}</p>}
                
            </div>

            {/* Footer Fijo (Siempre Visible) */}
            <div className="p-5 border-t border-[#222] bg-[#0a0a0a] flex gap-3 shrink-0">
                <Button onClick={onClose} variant="secondary" className="flex-1 !py-3 uppercase font-bold text-xs bg-[#111] border-[#222] text-zinc-400 hover:text-white">
                    Cancelar
                </Button>
                <Button onClick={handleSaveClick} className="flex-[2] !py-3 uppercase font-black text-xs bg-white text-black hover:bg-zinc-200 border-none">
