// components/CoachChatModal.tsx
import React, { useState, useRef, useEffect } from 'react';
import { Program, WorkoutLog, ChatMessage, Settings, GenerateContentResponse, Session, OngoingWorkoutState } from '../types';
...
interface CoachChatModalProps {
  ...
  isOnline: boolean;
  settings: Settings;
  sessionContext?: Session | OngoingWorkoutState;
}

const CoachChatModal: React.FC<CoachChatModalProps> = ({ isOpen, onClose, programs, history, isOnline, settings, sessionContext }) => {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const stream: AsyncGenerator<GenerateContentResponse> = await getCoachChatResponseStream(input, messages, programs, history, settings, sessionContext);
        ...
        <div className="flex flex-col h-[70vh] max-h-[500px]">
            ...
            <div className="flex-grow bg-slate-950/70 p-4 rounded-lg overflow-y-auto h-96 flex flex-col gap-4">
                {messages.length === 0 && (
                    <div className="m-auto text-center text-slate-400">
                        ...
                        <p className="text-sm">{sessionContext ? 'Pregúntame lo que necesites sobre esta sesión.' : 'Pregúntame cualquier cosa sobre tus programas o tu progreso.'}</p>
                    </div>
                )}
                {messages.map((msg, index) => (
                    <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-xs md:max-w-md p-3 rounded-lg ${msg.role === 'user' ? 'bg-primary-color text-white' : 'bg-slate-800 text-slate-200'}`}>
            ...
            <form onSubmit={handleSend} className="mt-4 flex gap-2">
                <input
                    ...
                    className="flex-grow bg-slate-800 border border-slate-700 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-color disabled:opacity-50"
                />
