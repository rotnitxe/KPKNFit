...
const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ checked, onChange, label, size = 'md', isBlackAndWhite = false }) => {
    ...
    // White theme with Material Design styling
    const bgClass = checked
        ? (isBlackAndWhite ? 'bg-slate-800' : 'bg-cyan-500')
        : 'bg-slate-300';

    const knobClass = 'bg-white';

    return (
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => onChange(!checked)}>
            {label && <span className={`font-medium text-slate-600 ${size === 'sm' ? 'text-xs' : 'text-sm'}`}>{label}</span>}
            <div className={`relative inline-flex flex-shrink-0 ${h} ${w} border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none ${bgClass} shadow-sm`}>
