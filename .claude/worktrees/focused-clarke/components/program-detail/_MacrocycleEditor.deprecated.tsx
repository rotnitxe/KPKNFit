...
const toLoopEvent = (event: NonNullable<Program['events']>[number]): LoopEvent => ({
    id: event.id || crypto.randomUUID(),
    title: event.title,
    type: (event.type as LoopEvent['type']) || 'custom',
...
const MacrocycleEditor: React.FC<MacrocycleEditorProps> = ({
    ...
    const [loopEvents, setLoopEvents] = useState<LoopEvent[]>(
        (program.events || []).filter((event) => event.repeatEveryXCycles).map(toLoopEvent)
    );
    ...
    useEffect(() => {
        setLoopEvents((program.events || []).filter((event) => event.repeatEveryXCycles).map(toLoopEvent));
    ...
    const addLoop = () => {
        ...
        setLoopEvents([...loopEvents, newEvent]);
    };

    const updateLoop = (id: string, updates: Partial<LoopEvent>) => {
        setLoopEvents(loopEvents.map(e => e.id === id ? { ...e, ...updates } : e));
    };

    const deleteLoop = (id: string) => {
        setLoopEvents(loopEvents.filter(e => e.id !== id));
    };
    ...
    const handleSave = () => {
        ...
        if (isSimple) {
            // Guardar loops
            updated.events = loopEvents.map(e => ({
                id: e.id,
                title: e.title,
                type: e.type,
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-[10px] font-black uppercase tracking-[0.25em] text-zinc-500">
                                {isSimple ? 'Loops' : 'Fechas Clave'}
                            </h3>
                            <button
                                onClick={isSimple ? addLoop : addKeyDate}
                            ...
                            <div className="space-y-2">
                                {loopEvents.length === 0 && (
                                    <div className="py-6 text-center border-2 border-dashed border-[#ECE6F0] rounded-2xl">
                                        <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Sin loops configurados</p>
                                    </div>
                                )}
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <input
                                                        ...
                                                        onChange={(e) => updateLoop(event.id, { repeatEveryXCycles: parseInt(e.target.value) || 1 })}
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => updateLoop(event.id, { durationType: e.target.value as 'day' | 'week' })}
                                        ...
                                        {event.durationType === 'day' && (
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => updateLoop(event.id, { dayOfWeek: parseInt(e.target.value) })}
                                                    className="w-full h-10 rounded-xl bg-[#F5F3F8] border border-[#ECE6F0] text-sm font-bold outline-none focus:border-purple-400"
                                                >
                            ...
                            <div className="bg-[#F5F3F8] rounded-2xl p-5 mb-6 space-y-3">
                                <div className="flex items-start gap-3">
                                    ...
                                    <p className="text-[9px] text-zinc-600 font-bold leading-relaxed">
                                        Se eliminarán todos los <span className="font-black">loops</span> configurados
                                    </p>
                                </div>
                                <div className="flex items-start gap-3">
