...
import type { WorkoutLog, ExerciseMuscleInfo, Settings, MuscleGroupInfo, ArticularBatteryId, ArticularBatteryState } from '@kpkn/shared-types';
...
export function calculateArticularBatteries(
  history: WorkoutLog[],
  dependencies: {
    buildExerciseIndex: (list: ExerciseMuscleInfo[]) => any;
    findExerciseWithFallback: (index: any, id?: string, name?: string) => ExerciseMuscleInfo | undefined;
    getArticularBatteriesForExercise: (info: ExerciseMuscleInfo | undefined, muscleGroupData: MuscleGroupInfo[]) => Record<ArticularBatteryId, number>;
    calculateSetTendonDrain: (set: any, info: ExerciseMuscleInfo | undefined, weights: Record<ArticularBatteryId, number>) => Record<ArticularBatteryId, number>;
  },
...
/** Mapeo muscle name → baterías articulares relacionadas */
