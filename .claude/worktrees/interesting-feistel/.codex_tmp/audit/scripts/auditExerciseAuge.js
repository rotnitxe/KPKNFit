...
function getCanonicalCatalog() {
    const names = INITIAL_MUSCLE_GROUP_DATA.map((item) => item.name);
...
function auditRepresentativeDrain(exercises) {
    ...
    return exercises.map((exercise) => ({
        ...
        drains: scenarios.map((scenario) => ({
            ...
            drain: calculateSetBatteryDrain(scenario.set, exercise, tanks, 1, scenario.restTime),
        })),
    }));
}
...
const fullList = FULL_EXERCISE_LIST;
