    ...
    if (mealTemplates.length === 0) {
        return (
            <div className="p-4 text-center">
                ...
                <p className="text-sm text-[#49454F]">No hay plantillas guardadas.</p>
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
            {mealTemplates.map((t) => (
                <button
                    ...
                    className="w-full text-left p-3 rounded-xl bg-white/5 border border-[#E6E0E9] hover:bg-white/10 hover:border-white/20 transition-all"
                >
                    <div className="flex justify-between items-start">
                        <span className="font-bold text-white text-sm">{t.name}</span>
                        <span className="text-[10px] font-mono text-[#49454F]">{t.totalCalories} kcal</span>
                    </div>
                    <p className="text-[10px] text-[#49454F] mt-0.5 truncate">{t.description}</p>
