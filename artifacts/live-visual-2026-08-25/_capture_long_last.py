#!/usr/bin/env python3
"""Capture long last set after readiness dismiss; advance via card arrows."""
...
def dismiss():
    for y in (2190, 2140, 2240, 2189, 2100, 2050):
...
def main():
    ...
    time.sleep(15)
    print("dismiss1", dismiss(), flush=True)
    pull("21-long-first-set.png")
    # Advance sets via right chevron on card
    for i in range(14):
        sh("input tap 1000 700")
        time.sleep(0.45)
        if ready(pull("_probe.png")):
            print("readiness mid-advance; dismiss", flush=True)
            dismiss()
    # Also tap lower rail nodes (later sets)
    for y in range(900, 1500, 40):
        sh(f"input tap 80 {y}")
        time.sleep(0.15)
    time.sleep(0.8)
    if ready(pull("_probe.png")):
    ...
    print(
        ...
        "ready22",
        ready(OUT / "22-long-last-set.png"),
        flush=True,
    )
