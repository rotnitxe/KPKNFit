#!/usr/bin/env python3
"""Cold-start slow isthmus P0 capture."""
...
def pill_y(path: Path) -> tuple[int | None, int]:
    im = Image.open(path).convert("RGB")
    best = 0
    by = None
...
def dismiss_ready() -> bool:
    for i in range(12):
        if anr_text():
            print("ANR -> Wait", flush=True)
            tap_cd("Wait") or sh("input tap 700 1280")
            time.sleep(4)
            continue
        p = shot("_probe.png")
        gr, nr = gn(p)
        print("probe", gr, nr, flush=True)
...
def open_session():
    ...
    print("cold wait 28s", flush=True)
    time.sleep(28)


def main():
    open_session()
    assert dismiss_ready(), "no live"
    time.sleep(2.0)

    p = shot("40-isthmus-s1-clean.png")
    print("S1", pill_y(p), gn(p), hashlib.md5(p.read_bytes()).hexdigest()[:8], flush=True)
    ...
    print("S3", pill_y(p), gn(p), hashlib.md5(p.read_bytes()).hexdigest()[:8], "anr", anr_text(), flush=True)
    py, _ = pill_y(p)
    if abs((py or 0) - 513) > 90 and not anr_text():
        sh("input tap 89 513")
        time.sleep(1.8)
        p = shot("41-isthmus-s3-clean.png")
        print("S3 rail", pill_y(p), gn(p), hashlib.md5(p.read_bytes()).hexdigest()[:8], flush=True)
    ...
    print("LAST", pill_y(p), gn(p), hashlib.md5(p.read_bytes()).hexdigest()[:8], "anr", anr_text(), flush=True)
