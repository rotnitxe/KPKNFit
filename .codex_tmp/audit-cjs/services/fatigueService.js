...
exports.calculateCompletedSessionDrainBreakdown = exports.calculateCompletedSessionStress = exports.isSetEffective = exports.calculateExerciseFatigueScale = exports.normalizeToTenScale = exports.calculateSpinalScore = exports.calculateSetStress = exports.calculatePredictedSessionDrain = exports.calculateSetBatteryDrain = exports.calculatePersonalizedBatteryTanks = exports.getEffectiveRPE = exports.getDynamicAugeMetrics = exports.WEEKLY_CNS_FATIGUE_REFERENCE = void 0;
const exerciseIndex_1 = require("../utils/exerciseIndex");
const inferMusclesFromName_1 = require("../data/inferMusclesFromName");
...
const getDynamicAugeMetrics = (info, customName) => {
    ...
    if (name.includes('peso muerto') || name.includes('deadlift')) {
        ...
        if (name.includes('búlgara') || name.includes('bulgarian')) {
            efc = 3.8;
            ssc = 0.8;
            cnc = 3.5;
        }
...
exports.getDynamicAugeMetrics = getDynamicAugeMetrics;
const getEFC = (info) => {
    return (0, exports.getDynamicAugeMetrics)(info).efc;
};
const getEffectiveRPE = (set) => {
    let baseRpe = 7; // Valor por defecto
    // 1. Traductor Universal RIR a RPE
    if (set.completedRPE !== undefined)
        baseRpe = set.completedRPE;
    else if (set.targetRPE !== undefined)
        baseRpe = set.targetRPE;
    else if (set.completedRIR !== undefined)
        baseRpe = 10 - set.completedRIR;
    else if (set.targetRIR !== undefined)
        baseRpe = 10 - set.targetRIR;
    ...
    if (set.dropSets && set.dropSets.length > 0)
        techniqueBonus += set.dropSets.length * 1.5;
...
exports.getEffectiveRPE = getEffectiveRPE;
/**
 ...
 */
const calculatePersonalizedBatteryTanks = (settings) => {
    ...
    if (style.includes('powerlift')) {
        cnsMult = 1.3;
        spineMult = 1.4;
        muscMult = 0.9; // SNC y Espalda de acero, menos tolerancia metabólica
    }
    else if (style.includes('bodybuild') || style.includes('aesthetics')) {
        cnsMult = 0.9;
        spineMult = 0.9;
        muscMult = 1.3; // Alta tolerancia al ácido láctico y volumen local
    }
    else {
        cnsMult = 1.15;
        spineMult = 1.15;
        muscMult = 1.15; // Powerbuilder / Híbrido
    }
    return {
...
exports.calculatePersonalizedBatteryTanks = calculatePersonalizedBatteryTanks;
const getPrimaryDisplayMuscle = (info, exerciseName, equipment = '') => {
    const primary = info?.involvedMuscles?.find(m => m.role === 'primary');
    if (primary) {
        return (0, canonicalMuscles_1.getMuscleDisplayId)(primary.muscle, primary.emphasis);
    }
    const inferred = (0, inferMusclesFromName_1.inferInvolvedMuscles)(exerciseName ?? '', equipment ?? '', 'Otro', 'upper')[0];
    return inferred ? (0, canonicalMuscles_1.getMuscleDisplayId)(inferred.muscle, inferred.emphasis) : 'Core';
};
const shouldSkipCompletedSet = (set) => {
    if (!set)
        return true;
    if (set.type === 'warmup' || set.isIneffective)
        return true;
    return !(0, exports.isSetEffective)(set);
};
/**
 ...
 */
const calculateSetBatteryDrain = (set, info, tanks, accumulatedSetsForMuscle = 0, restTime = 90) => {
    if (set?.type === 'warmup' || set?.isIneffective) {
        return {
            muscularDrainPct: 0,
            cnsDrainPct: 0,
    ...
    const auge = (0, exports.getDynamicAugeMetrics)(info, set.exerciseName || info?.name);
    const rpe = (0, exports.getEffectiveRPE)(set);
    ...
    if (rpe >= 11)
        intensityMult = 1.8; // Más allá del fallo (Dropsets/RestPause)
    else if (rpe >= 10)
        intensityMult = 1.5; // Fallo Absoluto (Exponencial)
    else if (rpe >= 9)
        intensityMult = 1.15; // RIR 1
    else if (rpe >= 8)
        intensityMult = 1.0; // RIR 2 (Base óptima)
    else if (rpe >= 6)
        intensityMult = 0.7; // RIR 3-4 (Bombeo)
    else
        intensityMult = 0.4; // RIR 5+ (Calentamiento/Recuperación)
    ...
    let restFactor = 1.0;
    if (restTime <= 45)
        restFactor = 1.3; // Superseries drenan más rápido
    else if (restTime >= 180)
        restFactor = 0.85; // Descansos largos protegen el SNC
    ...
    const rawMuscular = auge.efc * repsMuscMult * intensityMult * junkVolumeMult * restFactor * junkVolumeFromPartials * 7.2;
    const rawCns = auge.cnc * repsCnsMult * intensityMult * cnsRestFactor * compoundNeuralFactor * advancedTechniqueNeuralFactor * 3.4;
...
exports.calculateSetBatteryDrain = calculateSetBatteryDrain;
/**
 * --- PREDICTOR GLOBAL DE LA SESIÓN ---
 * Suma todos los porcentajes para dar el total de batería que costará la sesión.
 */
const calculatePredictedSessionDrain = (session, exerciseList, settings) => {
    const tanks = (0, exports.calculatePersonalizedBatteryTanks)(settings);
    const exIndex = (0, exerciseIndex_1.buildExerciseIndex)(exerciseList);
    let totalCnsPct = 0;
    ...
    exercises?.forEach(ex => {
        const info = (0, exerciseIndex_1.findExerciseWithFallback)(exIndex, ex.exerciseDbId, ex.name ?? ex.exerciseName);
        const primaryMuscle = getPrimaryDisplayMuscle(info, ex.name ?? ex.exerciseName ?? '', ex.equipment ?? '');
        ...
        ex.sets?.forEach(s => {
            ...
            const drain = (0, exports.calculateSetBatteryDrain)(s, info, tanks, accumulatedSets, ex.restTime || 90);
...
exports.calculatePredictedSessionDrain = calculatePredictedSessionDrain;
// Funciones Legacy Mantenidas para Retrocompatibilidad temporal en otras vistas
const calculateSetStress = (set, info, restTime = 90) => {
    const defaultTanks = (0, exports.calculatePersonalizedBatteryTanks)({});
    const drain = (0, exports.calculateSetBatteryDrain)(set, info, defaultTanks, 0, restTime);
    return drain.muscularDrainPct;
};
exports.calculateSetStress = calculateSetStress;
const calculateSpinalScore = (set, info) => {
    const defaultTanks = (0, exports.calculatePersonalizedBatteryTanks)({});
    const drain = (0, exports.calculateSetBatteryDrain)(set, info, defaultTanks, 0, 90);
    return drain.spinalDrainPct;
};
exports.calculateSpinalScore = calculateSpinalScore;
const normalizeToTenScale = (verroScore) => {
    return Math.min(10, Math.max(1, parseFloat(((verroScore / 10) * 10).toFixed(1)))); // Aproximación
};
exports.normalizeToTenScale = normalizeToTenScale;
const calculateExerciseFatigueScale = (exercise, info) => {
    if (!exercise.sets?.length)
        return 0;
    const defaultTanks = (0, exports.calculatePersonalizedBatteryTanks)({});
    let totalDrain = 0;
    exercise.sets.forEach((s, idx) => {
        const drain = (0, exports.calculateSetBatteryDrain)(s, info, defaultTanks, idx, exercise.restTime || 90);
        totalDrain += drain.cnsDrainPct + (drain.muscularDrainPct * 0.5);
    });
    return Math.min(10, Math.round(totalDrain / 2));
};
exports.calculateExerciseFatigueScale = calculateExerciseFatigueScale;
const isSetEffective = (set) => {
    const rpe = (0, exports.getEffectiveRPE)(set);
    if (rpe > 6)
        return true;
    const rir = set.completedRIR ?? set.targetRIR;
    if (rir !== undefined && rir < 4)
        return true;
    if (set.isFailure || set.performanceMode === 'failure' || set.intensityMode === 'failure')
        return true;
    if (set.intensityMode === 'solo_rm')
        return true;
    if (set.restPauses?.length > 0 && set.restPauses.some((rp) => (rp.reps ?? 0) > 0))
        return true;
    if (set.dropSets?.length > 0 && set.dropSets.some((ds) => (ds.reps ?? 0) > 0))
        return true;
    if (set.isAmrap)
        return true;
    return rpe >= 6;
};
exports.isSetEffective = isSetEffective;
/**
...
exports.calculateCompletedSessionStress = calculateCompletedSessionStress;
/**
 ...
 */
const calculateCompletedSessionDrainBreakdown = (completedExercises, exerciseList, settings) => {
    const tanks = (0, exports.calculatePersonalizedBatteryTanks)(settings || {});
...
exports.calculateCompletedSessionDrainBreakdown = calculateCompletedSessionDrainBreakdown;
