...
exports.FULL_EXERCISE_LIST = exports.EXERCISE_ID_ALIASES = void 0;
const exerciseDatabaseCentral_1 = require("./exerciseDatabaseCentral");
/** Mapa de IDs antiguos (tren inferior) -> IDs nuevos (exerciseDatabaseCentral) para retrocompatibilidad */
const LOWER_BODY_ID_ALIASES = {
...
function enrichWithOperationalData(ex) {
    const hasCore = ex.involvedMuscles?.some(m => ['Core', 'Abdomen', 'Espalda Baja', 'Recto Abdominal', 'Transverso Abdominal'].includes(m.muscle) && (m.activation || 0) >= 0.3);
...
const merged = [...exerciseDatabaseCentral_1.UPPER_BODY_EXERCISES, ...exerciseDatabaseCentral_1.LOWER_BODY_EXERCISES, ...exerciseDatabaseCentral_1.ULTIMO_LOTE_EXERCISES];
...
const fullAliasMap = new Map(nameAliasMap);
for (const [oldId, newId] of Object.entries(LOWER_BODY_ID_ALIASES)) {
    if (!fullAliasMap.has(oldId))
        fullAliasMap.set(oldId, newId);
}
...
exports.EXERCISE_ID_ALIASES = fullAliasMap;
exports.FULL_EXERCISE_LIST = nameDeduped.map(enrichWithOperationalData);
