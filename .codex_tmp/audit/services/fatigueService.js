 ...
 */
export const getDynamicAugeMetrics = (info, customName) => {
...
export const getEffectiveRPE = (set) => {
 ...
 */
export const calculatePersonalizedBatteryTanks = (settings) => {
 ...
 */
export const calculateSetBatteryDrain = (set, info, tanks, accumulatedSetsForMuscle = 0, restTime = 90) => {
 ...
 */
export const calculatePredictedSessionDrain = (session, exerciseList, settings) => {
    ...
    exercises?.forEach(ex => {
        ...
        const primaryMuscle = info?.involvedMuscles?.find(m => m.role === 'primary')?.muscle
            || inferInvolvedMuscles(name, ex.equipment ?? '', 'Otro', 'upper')[0]?.muscle
            || 'Core';
...
export const calculateSetStress = (set, info, restTime = 90) => {
...
export const isSetEffective = (set) => {
    const rpe = getEffectiveRPE(set);
    if (rpe > 6)
        return true;
 ...
 */
export const calculateCompletedSessionStress = (completedExercises, exerciseList) => {
 ...
 */
export const calculateCompletedSessionDrainBreakdown = (completedExercises, exerciseList, settings) => {
    const tanks = calculatePersonalizedBatteryTanks(settings || {});
