# Índice del catálogo de ejercicios v2

Revisión: `v2-approved-2026-08-12-a` · Ontología: `wikilab-v3-2026-08-08`
96 familias · 195 definiciones · 515 configuraciones
Hash canónico: `57c2a362ffcc8cdb…`

Artefacto informativo generado por `scripts/merge_catalog_v2_families.py`.
No editar a mano: se regenera en cada merge.

| familia | definición | nombre | ejes | configs |
|---|---|---|---|---|
| back_chest_supported_row | chest_supported_row | Remo con Pecho Apoyado | implement, pulley_height, grip_width | 18 |
| back_conventional_row | conventional_row | Remo Convencional | implement | 6 |
| back_gironda_row | gironda_row | Remo Gironda | grip_width | 3 |
| back_pendlay_row | pendlay_row | Remo Pendlay | implement | 6 |
| back_seal_row | seal_row | Remo Seal | implement | 2 |
| back_t_bar_row | t_bar_row | Remo en Barra T | implement, grip_width | 6 |
| chest_floor_press | floor_press | Floor Press | implement | 2 |
| chest_fly | decline_chest_fly | Aperturas Declinadas | implement | 3 |
| chest_fly | flat_chest_fly | Aperturas Planas | implement | 3 |
| chest_fly | incline_chest_fly | Aperturas Inclinadas | implement | 3 |
| chest_fly | reverse_pec_fly | Aperturas Inversas | implement, laterality | 6 |
| chest_press | bench_press | Press de Banca Plano | implement | 6 |
| chest_press | decline_bench_press | Press de Banca Declinado | implement | 6 |
| chest_press | incline_bench_press | Press de Banca Inclinado | implement | 6 |
| core_anti_extension_isometric | core_plancha | Plancha Abdominal |  | 1 |
| core_anti_extension_pelvic_control | core_dragon_flag_banco_plano | Dragon Flag |  | 1 |
| core_anti_extension_trunk | core_rueda_abdominal | Rueda Abdominal |  | 1 |
| core_anti_rotation_trunk | core_press_pallof | Press Pallof |  | 1 |
| core_hip_flexion | core_elevacion_piernas | Elevación de Piernas Colgado |  | 1 |
| core_lateral_trunk_flexion | core_inclinacion_lateral | Inclinación Lateral |  | 1 |
| core_trunk_flexion | core_crunch_banco_declinado_lastrado_disco | Crunch Abdominal en Banco Declinado |  | 1 |
| core_trunk_flexion | core_crunch_en_polea_alta | Crunch Abdominal de Rodillas en Polea Alta |  | 1 |
| core_trunk_flexion | core_crunch_maquina | Crunch Abdominal en Máquina |  | 1 |
| core_trunk_flexion | core_crunch_suelo_peso_corporal | Crunch Abdominal en Suelo |  | 1 |
| core_trunk_rotation | core_lenador_polea | Leñador en Polea |  | 1 |
| elbow_flexion_biceps_curl | biceps_curl_bayesian | Curl Bayesian | implement, grip_type | 6 |
| elbow_flexion_biceps_curl | biceps_curl_crucifijo | Curl Crucifijo |  | 1 |
| elbow_flexion_biceps_curl | biceps_curl_drag | Curl Drag |  | 1 |
| elbow_flexion_biceps_curl | biceps_curl_sentado_banco_plano | Curl de Bíceps Sentado | implement | 2 |
| elbow_flexion_biceps_curl | biceps_curl_superman | Curl Superman |  | 1 |
| elbow_flexion_biceps_curl | biceps_curl_trx | Curl de Bíceps en TRX |  | 1 |
| elbow_flexion_biceps_curl | biceps_curl_waiter | Curl Waiter |  | 1 |
| elbow_flexion_biceps_curl | biceps_curl_zottman | Curl Zottman |  | 1 |
| elbow_flexion_biceps_curl | concentration_curl | Curl Concentrado | implement | 2 |
| elbow_flexion_biceps_curl | preacher_curl | Curl Predicador | implement | 4 |
| elbow_flexion_biceps_curl | spider_curl | Curl Araña | implement, grip_type | 9 |
| elbow_flexion_biceps_curl | standing_biceps_curl | Curl de Bíceps de Pie | implement | 4 |
| elbow_flexion_biceps_curl | hammer_curl | Curl Martillo | implement | 5 |
| elbow_flexion_biceps_curl | reverse_curl | Curl Invertido | implement | 4 |
| hinge_deadlift | conventional_deadlift | Peso Muerto Convencional | implement, laterality | 8 |
| hinge_deadlift | sumo_deadlift | Peso Muerto Sumo | implement | 2 |
| hinge_good_morning | good_morning | Buenos Días | implement, laterality | 8 |
| hinge_good_morning | good_morning_seated | Buenos Días Sentado | implement | 3 |
| hinge_good_morning | good_morning_zercher | Buenos Días/RDL Zercher |  | 1 |
| hinge_rdl | romanian_deadlift | Peso Muerto Rumano | implement, stance | 8 |
| hinge_rdl | romanian_sumo_deadlift | Peso Muerto Rumano Sumo | implement, stance | 8 |
| hip_abduction | hip_abduction | Abducciones de Pierna | implement, station, laterality | 8 |
| hip_adduction | copenhagen_plank | Plancha Copenhague |  | 1 |
| hip_adduction | hip_adduction | Aducciones de Pierna | implement, station, laterality | 8 |
| hip_adduction | copenhagen_plank_dynamic | Plancha Copenhague Dinámica |  | 1 |
| lower_ankle_dorsiflexion | calves_tibial_anterior | Tibial Anterior |  | 1 |
| lower_biarticular_lengthened | quads_reverse_nordic_peso_corporal | Curl Nórdico Inverso (Reverse Nordic Curl) |  | 1 |
| lower_eccentric_knee_flexion | hams_curl_nordic_peso_corporal | Curl Nórdico (Nordic Hamstring Curl) |  | 1 |
| lower_forward_lunge | forward_lunge | Zancada Frontal | implement | 5 |
| lower_hip_abduction_extension | glutes_patada_gluteo_lateral | Patada de Glúteo Lateral | implement | 3 |
| lower_hip_abduction_external_rotation | glutes_clamshells_banda | Clamshells (Almejas) con Banda Elástica |  | 1 |
| lower_hip_abduction_stability | glutes_monster_walk_banda | Caminata del Monstruo (Monster Walk) con Banda |  | 1 |
| lower_hip_extension | glutes_hiperextension_45 | Hiperextensiones a 45° para Glúteos | implement | 4 |
| lower_hip_extension | glutes_patada_gluteo | Patada de Glúteo | implement | 3 |
| lower_hip_extension | glutes_puente_gluteos | Puente de Glúteos | implement, laterality | 6 |
| lower_hip_extension | reverse_hyper | Reverse Hyper |  | 1 |
| lower_hip_extension | glutes_hiperextension_45_zercher | Hiperextensión a 45 Zercher para Glúteos |  | 1 |
| lower_hip_extension_abduction | glutes_patada_gluteo_polea_diagonal | Patada de Glúteos Diagonal en Polea |  | 1 |
| lower_hip_extension_external_rotation | glutes_frog_pumps | Frog Pumps |  | 1 |
| lower_hip_extension_hip_thrust | hip_thrust | Hip Thrust | implement, laterality | 8 |
| lower_hip_hinge | hams_pull_through | Pull-Through |  | 1 |
| lower_hip_hinge_deficit | hams_peso_muerto_convencional_deficit | Peso Muerto Convencional en Déficit |  | 1 |
| lower_hip_hinge_deficit | hams_peso_muerto_piernas_rigidas_deficit | Peso Muerto Piernas Rígidas en Déficit |  | 1 |
| lower_hip_hinge_deficit | hams_peso_muerto_sumo_deficit | Peso Muerto Sumo en Déficit |  | 1 |
| lower_hip_hinge_explosive | hams_swing_kettlebell_dos_manos | Swing con Kettlebell a Dos Manos |  | 1 |
| lower_hip_hinge_explosive | hams_swing_kettlebell_unilateral | Swing con Kettlebell a Una Mano |  | 1 |
| lower_hip_hinge_lengthened | stiff_leg_deadlift | Peso Muerto Piernas Rígidas | implement, laterality | 8 |
| lower_isometric_grip | forearms_suspension_isometrica_barra_fija | Suspensión Isométrica en Barra Fija (Dead Hang) |  | 1 |
| lower_isometric_grip | forearms_paseo_del_granjero | Paseo del Granjero | implement | 4 |
| lower_knee_dominant | front_squat | Sentadilla Frontal | implement | 5 |
| lower_knee_dominant | high_bar_back_squat | Sentadilla Trasera con Barra Alta | implement | 3 |
| lower_knee_dominant | low_bar_back_squat | Sentadilla Trasera con Barra Baja | implement | 2 |
| lower_knee_dominant | quads_prensa_piernas | Prensa de Piernas | laterality | 2 |
| lower_knee_dominant | quads_sentadilla_anderson | Sentadilla Anderson |  | 1 |
| lower_knee_dominant | quads_sentadilla_anderson_frontal_barra_recta | Sentadilla Anderson Frontal |  | 1 |
| lower_knee_dominant | quads_sentadilla_bazuca | Sentadilla Bazuca |  | 1 |
| lower_knee_dominant | quads_sentadilla_copa | Sentadilla Copa |  | 1 |
| lower_knee_dominant | quads_sentadilla_hack | Sentadilla Hack | implement | 3 |
| lower_knee_dominant | quads_sentadilla_hack_invertida_maquina | Sentadilla Invertida en Máquina Hack |  | 1 |
| lower_knee_dominant | quads_sentadilla_somersault | Sentadilla Somersault |  | 1 |
| lower_knee_dominant | quads_sentadilla_v_squat | Sentadilla en "V-Squat" |  | 1 |
| lower_knee_dominant | quads_sentadilla_v_squat_invertida_maquina | Sentadilla Invertida en Máquina V-Squat |  | 1 |
| lower_knee_dominant | quads_sentadilla_zercher_barra_recta | Sentadilla Zercher con Barra Recta |  | 1 |
| lower_knee_dominant | sumo_squat | Sentadilla Sumo | implement | 3 |
| lower_knee_dominant_asymmetric | quads_sentadilla_jefferson | Sentadilla Jefferson |  | 1 |
| lower_knee_dominant_belt_squat | belt_squat | Sentadilla "Belt Squat" | laterality | 2 |
| lower_knee_dominant_pendulum | pendulum_squat | Sentadilla en Máquina Pendular | laterality | 2 |
| lower_knee_extension | quads_extension_cuadriceps | Extensión de Cuádriceps en Máquina | laterality | 2 |
| lower_knee_extension | quads_extension_cuadriceps_pie_polea | Extensión de Cuádriceps de Pie en Polea | laterality | 2 |
| lower_knee_flexion | curl_isquios_con_balon | Curl de Isquiosurales con Balón |  | 1 |
| lower_knee_flexion | curl_isquios_con_sliders | Curl de Isquiosurales con Sliders |  | 1 |
| lower_knee_flexion | lying_leg_curl | Curl de Isquiosurales Tumbado | implement, laterality | 6 |
| lower_knee_flexion | seated_leg_curl | Curl de Isquiosurales Sentado | implement, laterality | 4 |
| lower_knee_flexion | standing_leg_curl | Curl de Isquiosurales de Pie | implement, laterality | 4 |
| lower_knee_hip_dominant | quads_sentadilla_cajon | Sentadilla a Cajón |  | 1 |
| lower_knee_hip_dominant | quads_sentadilla_sumo_frontal | Sentadilla Sumo Frontal |  | 1 |
| lower_knee_hip_dominant | quads_sentadilla_sumo_zercher | Sentadilla Sumo Zercher |  | 1 |
| lower_knee_hip_extension | glute_ham_raise | Glute Ham Raise |  | 1 |
| lower_lateral_knee_dominant | quads_sentadilla_cosaca | Sentadilla Cosaca |  | 1 |
| lower_neck_extension | neck_extension_cuello | Extensiones de Cuello | implement | 2 |
| lower_neck_flexion | neck_flexion_cuello | Flexiones de Cuello | implement | 2 |
| lower_neck_lateral_flexion | neck_flexion_lateral_cuello | Flexión Lateral de Cuello |  | 1 |
| lower_plantar_flexion | calf_raise | Elevación de Talones | implement, laterality | 8 |
| lower_reverse_lunge | reverse_lunge | Zancada Inversa | implement | 5 |
| lower_romanian_deadlift | hams_peso_muerto_rumano_deficit | Peso Muerto Rumano en Déficit |  | 1 |
| lower_romanian_deadlift_deficit | hams_peso_muerto_rumano_sumo_deficit | Peso Muerto Rumano Sumo en Déficit |  | 1 |
| lower_sissy_squat | sissy_squat | Sentadilla Sissy | implement | 5 |
| lower_spinal_extension | back_extension_lumbar | Hiperextensiones de Espalda Baja |  | 1 |
| lower_spinal_extension | back_superman_suelo | Superman en Suelo |  | 1 |
| lower_spinal_extension | back_hiperextension_45_zercher_espalda_baja | Hiperextensión a 45 Zercher para Espalda Baja |  | 1 |
| lower_spinal_flexion | back_jefferson_curl | Jefferson Curl | implement | 4 |
| lower_step_up | step_up | Step-Up a Cajón | implement | 5 |
| lower_unilateral_hip_dominant | glutes_step_up_gluteo | Step-Up de Glúteo |  | 1 |
| lower_unilateral_hip_dominant | glutes_zancada_cruzada | Zancada Cruzada |  | 1 |
| lower_unilateral_knee_dominant | quads_sentadilla_bulgara_somersault | Sentadilla Búlgara Somersault |  | 1 |
| lower_unilateral_knee_dominant | quads_sentadilla_pistola | Sentadilla Pistola |  | 1 |
| lower_unilateral_knee_dominant | quads_sentadilla_pistola_asistida_trx | Sentadilla Pistola Asistida en TRX |  | 1 |
| lower_unilateral_knee_dominant | quads_step_up_cajon_frontal | Step-Up a Cajón Frontal |  | 1 |
| lower_unilateral_knee_dominant | quads_step_up_cajon_zercher | Step-Up a Cajón Zercher |  | 1 |
| lower_unilateral_knee_dominant | quads_zancada_caminando_frontal_barra_recta | Zancada Caminando Frontal con Barra Recta |  | 1 |
| lower_unilateral_knee_dominant | quads_zancada_caminando_zercher_barra_recta | Zancada Caminando Zercher con Barra Recta |  | 1 |
| lower_unilateral_knee_dominant | quads_zancada_frontal_zercher | Zancada Frontal Zercher |  | 1 |
| lower_unilateral_knee_dominant | quads_zancada_inversa_frontal | Zancada Inversa Frontal |  | 1 |
| lower_unilateral_knee_dominant | quads_zancada_inversa_maquina_hack | Zancada Inversa en Máquina Hack |  | 1 |
| lower_unilateral_knee_dominant | quads_zancada_inversa_maquina_v_squat | Zancada Inversa en Máquina V-Squat |  | 1 |
| lower_unilateral_knee_dominant | quads_zancada_inversa_zercher | Zancada Inversa Zercher |  | 1 |
| lower_unilateral_knee_dominant_asymmetric | quads_sentadilla_bulgara_jefferson | Sentadilla Búlgara Jefferson |  | 1 |
| lower_walking_lunge | walking_lunge | Zancada Caminando | implement | 5 |
| lower_wrist_extension | forearms_curl_muneca_inverso_sentado | Extensión de Muñeca | implement | 4 |
| lower_wrist_flexion | forearms_curl_muneca_de_pie_tras_espalda_barra | Curl de Muñeca de Pie Tras Espalda con Barra |  | 1 |
| lower_wrist_flexion | forearms_curl_muneca_sentado | Curl de Muñeca | implement | 4 |
| lower_wrist_flexion_extension | forearms_enrollamiento_muneca_rodillo | Enrollamiento de Muñeca con Rodillo y Cuerda |  | 1 |
| lower_wrist_rotation | supination | Supinaciones | implement | 2 |
| lower_wrist_rotation | pronation | Pronaciones | implement | 2 |
| shoulder_arnold_press | arnold_press | Press Arnold | implement | 3 |
| shoulder_lateral_raise | seated_lateral_raise | Elevaciones Laterales Sentado | implement | 4 |
| shoulder_lateral_raise | standing_lateral_raise | Elevaciones Laterales de Pie | implement | 4 |
| shoulder_lateral_raise | lateral_raise_super_rom | Elevaciones Laterales Super ROM | implement | 3 |
| shoulder_rear_delt_raise | rear_delt_raise | Elevaciones Posteriores | implement | 3 |
| shoulder_z_press | z_press | Press Z | implement | 4 |
| triceps_california_press | california_press | Press California | implement | 3 |
| triceps_jm_press | jm_press | JM Press | implement | 5 |
| triceps_katana_extension | katana_extension | Extensión Katana | laterality | 2 |
| triceps_tate_press | tate_press | Tate Press |  | 1 |
| unilateral_knee_dominant_bulgarian | bulgarian_split_squat | Sentadilla Búlgara | implement | 6 |
| unilateral_knee_dominant_bulgarian | bulgarian_zercher | Sentadilla Búlgara Zercher |  | 1 |
| upper_diagonal_push | deltoides_press_landmine_unilateral | Press Landmine Unilateral |  | 1 |
| upper_elbow_extension | triceps_extension | Extensión de Tríceps en TRX |  | 1 |
| upper_elbow_extension | triceps_extension_pjr_mancuerna | Extensión de Tríceps PJR con Mancuerna |  | 1 |
| upper_elbow_extension | triceps_flexiones_esfinge | Flexiones Esfinge |  | 1 |
| upper_elbow_extension | triceps_fondos_entre_bancos | Fondos Entre Bancos |  | 1 |
| upper_elbow_extension | triceps_press_frances | Press Francés | implement | 5 |
| upper_elbow_extension | triceps_press_maquina | Press de Tríceps en Máquina |  | 1 |
| upper_elbow_extension | triceps_pushdown | Extensión de Tríceps | implement, laterality | 6 |
| upper_elbow_extension | triceps_rolling_extension | Rolling Extension de Tríceps |  | 1 |
| upper_elbow_extension_crossbody | crossbody_triceps_extension | Extensión de Tríceps Cruzada en Polea Alta | laterality | 2 |
| upper_elbow_extension_kickback | triceps_patada | Patada de Tríceps | implement, laterality | 3 |
| upper_elbow_extension_overhead | overhead_triceps_extension | Extensión de Tríceps Overhead | implement | 4 |
| upper_horizontal_pull | back_band_pull_apart | Band Pull-Apart |  | 1 |
| upper_horizontal_pull | back_remo_banda | Remo en Banda Elástica |  | 1 |
| upper_horizontal_pull | back_remo_gorilla_mancuernas | Remo Gorilla | implement | 3 |
| upper_horizontal_pull | back_remo_invertido | Remo Invertido |  | 1 |
| upper_horizontal_pull | back_remo_renegado_mancuernas | Remo Renegado | implement | 2 |
| upper_horizontal_pull | deltoides_face_pull | Face Pull |  | 1 |
| upper_horizontal_push | push_up | Flexiones de Brazos | support_angle | 2 |
| upper_horizontal_push | tren_superior_cruce_poleas | Cruce de Poleas | implement, pulley_height | 3 |
| upper_horizontal_push | tren_superior_fondos | Fondos en Paralelas |  | 1 |
| upper_horizontal_push | tren_superior_press_banca_cadenas | Press de Banca con Cadenas |  | 1 |
| upper_horizontal_push | tren_superior_press_banda_resistencia | Press con Banda de Resistencia |  | 1 |
| upper_horizontal_push | tren_superior_press_pecho_maquina_convergente | Press Plano en Máquina Convergente |  | 1 |
| upper_horizontal_push | tren_superior_press_spoto_barra | Press Spoto con Barra |  | 1 |
| upper_horizontal_push | tren_superior_press_unilateral_polea | Press Unilateral en Polea |  | 1 |
| upper_horizontal_push | tren_superior_squeeze_press_mancuernas | Squeeze Press con Mancuernas |  | 1 |
| upper_horizontal_push | tren_superior_press_inclinado_maquina_convergente | Press Inclinado en Máquina Convergente |  | 1 |
| upper_pinch_grip | forearms_pinza_de_discos | Pinza de Discos |  | 1 |
| upper_scapular_depression | back_dominadas_escapulares | Dominadas Escapulares |  | 1 |
| upper_scapular_elevation | back_encogimientos | Encogimientos | implement | 5 |
| upper_scapular_elevation | back_encogimientos_kelso | Kelso Shrugs | implement | 5 |
| upper_scapular_elevation | back_y_raises | Y-Raises |  | 1 |
| upper_shoulder_abduction_diagonal | deltoides_y_raises_sentado_banco_inclinado | Y-Raises Sentado en Banco Inclinado |  | 1 |
| upper_shoulder_abduction_full_rom | deltoides_lu_raises | LU-Raises |  | 1 |
| upper_shoulder_flexion | deltoides_elevaciones_frontales | Elevaciones Frontales | implement | 4 |
| upper_vertical_pull_abduction | deltoides_remo_menton | Remo al Mentón |  | 1 |
| upper_vertical_pull_lat_pulldown | lat_pulldown | Jalón al Pecho | implement, laterality | 6 |
| upper_vertical_pull_pull_up | pull_up | Dominadas | grip_type, grip_width | 9 |
| upper_vertical_pull_pullover | lying_pullover | Pull Over en Banca | implement | 5 |
| upper_vertical_pull_pullover | pullover | Pull Over | implement, laterality | 4 |
| upper_vertical_push | deltoides_push_press | Push Press |  | 1 |
| upper_vertical_push_military_press | military_press | Press Militar | implement | 6 |
| upper_vertical_push_seated_press | seated_shoulder_press | Press de Hombros Sentado | implement | 6 |
